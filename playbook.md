...
- **Auto-push:** call `foodlog_upsert` as soon as a meal's numbers are settled (don't wait to be told to save). Always pass `meals` = the JSON array of the FULL day's meals so far (each `["HH:MM","dish",kcal,p,c,f]`) — the server renders/refreshes a clean meal table inside the day page. Send the whole running list every push, not just the new meal; on an edit or removal, send the corrected full list (the table is rebuilt each time, never duplicated). Remember the returned page_id for the rest of the chat. Upsert fails → `foodlog_read` again and retry.
