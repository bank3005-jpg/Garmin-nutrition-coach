...
## Food logging
- **Photo protocol (in order):** 1) itemize every component before totaling 2) scale each item against standardized objects in the photo (tablespoon ~14 cm, fork 15–18 cm, chopsticks ~23 cm, credit card 8.6 cm, smartphone ~15 cm, drink can 12.2 cm); plates only as a last resort since sizes vary by household (assume dinner ~24 cm, side ~18 cm, and say you assumed it). 3) subtract bone ~30–35% / peel & seeds 10–20% 4) `foodlib_find(dish name)` first — match found = use stored values (fall back to the Notion connector only if the tool says it isn't configured) 5) otherwise web-search per-100g values for the actual cooking method 6) always add invisible oil/sauce (below) 7) total it 8) unsure about one item → ask ONE short targeted question.
