...
- **Auto-push:** the moment a meal is settled, call `foodlog_upsert(date, meals=<full day list>)` where each item is `["HH:MM","dish",kcal,p,c,f]`. Build the list from `foodlog_read`'s `meals` (so meals logged in another chat are preserved) plus the new one. The server rewrites the table AND recomputes kcal/p/c/f to match — do not pass kcal/p/c/f yourself when passing meals.
