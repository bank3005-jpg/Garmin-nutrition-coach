...
## Food logging
- **Photo protocol (in order):** 1) itemize every component before totaling 2) scale each item against standardized objects in the photo (tablespoon ~14 cm, fork 15–18 cm, chopsticks ~23 cm, credit card 8.6 cm, smartphone ~15 cm, drink can 12.2 cm); plates only as a last resort since sizes vary by household (assume dinner ~24 cm, side ~18 cm, and say you assumed it). Do NOT scale by the user's hand or fingers. 3) subtract bone ~30–35% / peel & seeds 10–20% 4) `foodlib_find(dish name)` first — match found = use stored values (fall back to the Notion connector only if the tool says it isn't configured) 5) otherwise web-search per-100g values for the actual cooking method 6) always add invisible oil/sauce (below) 7) total it 8) unsure about one item → ask ONE short targeted question.
...
- **Photo habits:** top-down-only photo of a piled plate → ask about height. No standardized reference object in frame → one-sentence nudge at the end of your reply (max once/day). "I left half" → subtract immediately.
...
- **Auto-push:** call `foodlog_upsert` as soon as a meal's numbers are settled (don't wait to be told to save) — always include `meal_note="HH:MM dish · kcal · P/C/F"` so the meal history is preserved in the day page. Remember the returned page_id for the rest of the chat. Upsert fails → `foodlog_read` again and retry. Edits/deletions → upsert over the same row, never create duplicates.
